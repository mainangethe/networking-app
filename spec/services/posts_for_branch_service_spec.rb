...
describe PostsForBranchService do
  ...
  context '#call' do
    ...
    it 'returns post filtered by a branch' do
      not_included_posts
      category
    ...
    it 'returns post filtered by a category name' do
      not_included_posts
      category
      included_post = [] << post
    ...
    it 'returns post filtered by a category name and a search input' do
      not_included_posts
      category
      included_post = [] << post
      expect(PostsForBranchService.new({ category: 'arts', search: 'fun', branch: 'hobby' }).call).to eq included_post
    end
